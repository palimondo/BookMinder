...
## day-007.md — examined, cut declined, file untouched

day-007.md was scheduled for the same delete-only treatment under the premise that lines 6396-7658 are byte-identical to 1852-3114. **That premise is false, and the repair was stopped before any edit.** The file is unchanged at blob `2b1776656183cce5c73dd82a7c89eed6c69eeec0` (8,126 newlines; the last line has no trailing newline).

Measured structure: four pastes, with `Welcome to Claude Code!` banner boxes opening at lines 2, 1670, 3120 and 6214 — P1 = 1-1669, P2 = 1670-3119, P3 = 3120-6213, P4 = 6214-EOF. The real duplication is P2 against P4's prefix (6214-7663) at a constant offset of 4544, 1,450 lines each. `cmp` of the remembered ranges differs at char 11923; `diff` of the true ranges reports 8 hunks over 15 lines; the maximal byte-identical sub-run is only 2605-3115 ↔ 7149-7659 (511 lines). The second copy is a re-wrapped rendering, not a byte-duplicate, so the byte-identity argument that licensed the day-020 cut does not transfer.

At token level the two renderings are word-for-word identical — 8,102 versus 8,103 whitespace-collapsed tokens — differing by exactly one inserted token: the `>` author-turn marker present only in P4, at line 7660. Seven of the eight hunks are pure re-wrap and carry no information beyond wrap position.

That single token is where the cut collides with the corpus. Of 132 day-007 refs (P1 45, P2 66, P3 2, P4-duplicate 4, P4-tail 15; zero zone-crossing ranges), **all four refs inside the proposed delete range point at line 7660** — and they are precisely the refs that depend on the `>` marker for author attribution: `day-007.yaml` rule `d007-R16` carries `because_source: user-verbatim` with `evidence: day-007:L7660-L7661`, the same line is a `pairing_gems` entry, `disposition-pair-programming.md` dispositions it, and `provenance-verification-bookminder-2.md` records the quote as verified "exact @ day-007:L7660-L7661 (wrap)". Remapping them by offset sends them to line 3116, which holds the same words *without* the `>` — so the plain offset rule fails the per-ref positional byte-equality gate on 4 of 132 refs, and the surviving rendering would no longer show that the turn is the author's rather than the agent's. This is F1 of the repair plan happening in the concrete: the only line that verbatim-backs a `user-verbatim` quote is the line a normalization-driven dedup would delete.

The cut is recoverable — the day-020 design already has the mechanism, an appendix row in the remap table for variant windows that no ref used there — but choosing it is a design decision on a primary source, and day-020's own precedent is that such a decision gets an adversarial review before execution, not an amendment invented mid-run. Three adjudicable options, for the author, none taken here: (a) delete 6214-7663 but preserve the 15 variant lines verbatim in a labeled appendix and remap the four line-7660 refs to the appendix rather than by offset — zero tokens lost, 132/132 refs would pass, at the cost of P4's surviving tail (7664-EOF) losing its session banner and following P3 directly; (b) delete P2 (1670-3119) instead, keeping the complete banner-headed rendering that carries the author marker, at the cost of shifting 111 of 132 refs and inverting the keep-the-first-occurrence convention; (c) no cut — annotate the file with a header note recording the P2 ≡ P4-prefix equivalence and leave both renderings in place, paying 1,450 redundant lines to keep every rendering exactly as captured.
