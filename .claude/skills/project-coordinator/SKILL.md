...
- Commit after each meaningful state change. Push policy is environment-conditional: on ephemeral infra (remote container, CI) push immediately — it is the only durability; on a local machine pushing is the author's decision, reserved to him. `.coordination/threads.md` is the standing record.
