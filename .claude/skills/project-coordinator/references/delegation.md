...
- **Slice for concurrency.** A single workflow's cap is CPU-bound — observed as `min(16, nproc - 2)` on the remote platform, so a 4-core container gives 2. Slicing the units across several parallel workflows with an `args` array multiplies effective concurrency. Take an arg-sliced variant of the swarm script as the normal shape for large runs.
